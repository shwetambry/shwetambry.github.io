...
var defaultHelpText =
  "<p style='text-align:left';> <br> Having trouble to figure out how to proceed??<br>" +
...
$(document).ready(function () {
  ...
  $(".carry").keyup(function () {
    ...
    if (tmpAr[1] == "-") {
      $(this)
        .siblings("#n1" + $carID[$carID.length - 1])
        .css("color", "lavender");
    }
  });
...
function carryOrNot(idx, txt) {
  ...
  timeOutObj = setTimeout(function () {
    ...
    if (idx == 1) {
      ...
      timeOutObj = setTimeout(function () {
        changeCol(
          ["n1" + idx, "n2" + idx, "carry" + idx, "res" + idx],
  ...
  if (na >= b) {
    timeOutObj = setTimeout(function () {
      ...
      timeOutObj = setTimeout(function () {
        ...
        timeOutObj = setTimeout(function () {
          changeCol(
            ["carry" + idx, "n1" + idx, "n2" + idx, "res" + idx],
            ["white", "white", "white", "white"]
          );
          if (idx > 1) {
    ...
    timeOutObj = setTimeout(function () {
      txt +=
        "<p>It is not possible to take away " +
        b +
        " from " +
        na +
        ". So " +
        na +
        " will borrow from preceeding one which is " +
        precV +
        ".</p>";
      ...
      timeOutObj = setTimeout(function () {
        txt +=
          "<p> After giving one away, the preceeding one becomes " +
          (precV - 1) +
          " and the current one becomes 1" +
          na +
          ".</p>";
        if (idx == 3) {
          changeCol(
            ["n1" + (idx - 1), "n1" + idx, "carry" + (idx - 1), "carry" + idx],
...
function carryMult(idx, txt) {
  ...
  timeOutObj = setTimeout(function () {
    ...
    if (idx == 1) {
      ...
      timeOutObj = setTimeout(function () {
        changeCol(
          ["n1" + idx, "n23", "carry" + idx, "carry" + (idx - 1), "res" + idx],
          ["white", "white", "white", "white", "white"]
        );
        timeOutObj = setTimeout(function () {
          carryMult(idx - 1, txt);
...
function checkAns(n1, n2, op, inp) {
  var txt =
    '<p style="color:red;" >Incorrect answer!!! The correct answer is ' +
    correctAns(n1, n2, op) +
    "</p>";
  if (op == "+") {
    attemptA += 1;
